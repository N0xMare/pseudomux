...
struct InstalledLiveAttachInputPause {
    ...
    pause: Arc<LiveAttachInputApplyPause>,
}

#[cfg(test)]
#[derive(Debug, Default)]
struct OrderlyAttachEofBoundaryPause {
    reached: tokio::sync::Notify,
    release: tokio::sync::Notify,
}

#[cfg(test)]
struct InstalledOrderlyAttachEofBoundaryPause {
    identity: crate::handler::attach_support::ActiveAttachIdentity,
    pause: Arc<OrderlyAttachEofBoundaryPause>,
}

#[cfg(test)]
static LIVE_ATTACH_INPUT_APPLY_PAUSE: std::sync::Mutex<Option<InstalledLiveAttachInputPause>> =
    std::sync::Mutex::new(None);

#[cfg(test)]
static LIVE_ATTACH_INPUT_VALIDATION_PAUSE: std::sync::Mutex<Option<InstalledLiveAttachInputPause>> =
    std::sync::Mutex::new(None);

#[cfg(test)]
static BEFORE_ORDERLY_ATTACH_EOF_CONTROL_SEAL_PAUSE: std::sync::Mutex<
    Option<InstalledOrderlyAttachEofBoundaryPause>,
> = std::sync::Mutex::new(None);

#[cfg(test)]
static AFTER_ORDERLY_ATTACH_EOF_CONTROL_SEAL_PAUSE: std::sync::Mutex<
    Option<InstalledOrderlyAttachEofBoundaryPause>,
> = std::sync::Mutex::new(None);

#[cfg(test)]
static ORDERLY_ATTACH_EOF_RETAINED_INPUT_VALIDATION_PAUSE: std::sync::Mutex<
    Option<InstalledOrderlyAttachEofBoundaryPause>,
> = std::sync::Mutex::new(None);
...
fn install_live_attach_input_apply_pause(
    ...
    *LIVE_ATTACH_INPUT_APPLY_PAUSE
        .lock()
        .expect("live attach input pause lock") = Some(InstalledLiveAttachInputPause {
        identity,
...
fn install_live_attach_input_validation_pause(
    ...
    *LIVE_ATTACH_INPUT_VALIDATION_PAUSE
        .lock()
        .expect("live attach input validation pause lock") = Some(InstalledLiveAttachInputPause {
        identity,
        pause: Arc::clone(&pause),
    });
    pause
...
#[cfg(test)]
fn install_before_orderly_attach_eof_control_seal_pause(
    identity: crate::handler::attach_support::ActiveAttachIdentity,
) -> Arc<OrderlyAttachEofBoundaryPause> {
    install_orderly_attach_eof_boundary_pause(
        &BEFORE_ORDERLY_ATTACH_EOF_CONTROL_SEAL_PAUSE,
        identity,
    )
}

#[cfg(test)]
fn install_after_orderly_attach_eof_control_seal_pause(
    identity: crate::handler::attach_support::ActiveAttachIdentity,
) -> Arc<OrderlyAttachEofBoundaryPause> {
    install_orderly_attach_eof_boundary_pause(
        &AFTER_ORDERLY_ATTACH_EOF_CONTROL_SEAL_PAUSE,
        identity,
    )
}

#[cfg(test)]
fn install_orderly_attach_eof_retained_input_validation_pause(
    identity: crate::handler::attach_support::ActiveAttachIdentity,
) -> Arc<OrderlyAttachEofBoundaryPause> {
    install_orderly_attach_eof_boundary_pause(
        &ORDERLY_ATTACH_EOF_RETAINED_INPUT_VALIDATION_PAUSE,
        identity,
    )
}

#[cfg(test)]
fn install_orderly_attach_eof_boundary_pause(
    slot: &std::sync::Mutex<Option<InstalledOrderlyAttachEofBoundaryPause>>,
    identity: crate::handler::attach_support::ActiveAttachIdentity,
) -> Arc<OrderlyAttachEofBoundaryPause> {
    let pause = Arc::new(OrderlyAttachEofBoundaryPause::default());
    *slot.lock().expect("orderly attach EOF boundary pause lock") =
        Some(InstalledOrderlyAttachEofBoundaryPause {
            identity,
            pause: Arc::clone(&pause),
        });
    pause
...
#[cfg(test)]
async fn pause_at_orderly_attach_eof_boundary(
    slot: &std::sync::Mutex<Option<InstalledOrderlyAttachEofBoundaryPause>>,
    identity: crate::handler::attach_support::ActiveAttachIdentity,
) {
    let pause = {
        let mut installed = slot.lock().expect("orderly attach EOF boundary pause lock");
        installed
            .as_ref()
            .is_some_and(|installed| installed.identity == identity)
            .then(|| {
                installed
                    .take()
                    .expect("matching orderly EOF pause remains installed")
                    .pause
            })
    };
...
pub(crate) async fn forward_attach(
    ...
    let result = async {
        loop {
            ...
            let socket_reentry = process_attach_socket_messages(
                &mut decoder,
                &stream,
                &live_input,
                &closing,
            ...
            tokio::select! {
                ...
                result = read_socket_bytes(&stream, &mut decoder) => {
                    ...
                    let _ = process_attach_socket_messages(
                        &mut decoder,
                        &stream,
                        &live_input,
                        &closing,
...
#[cfg(any(unix, windows))]
#[allow(clippy::too_many_arguments)]
async fn prepare_orderly_attach_eof(
    decoder: &mut AttachFrameDecoder,
    socket_reentry: AttachSocketReentry,
    live_input: &LiveAttachInputContext,
    closing: &AtomicBool,
    attach_controls: &mut Option<mpsc::UnboundedReceiver<AttachControl>>,
    deferred_controls: &mut VecDeque<AttachControl>,
    control_backlog: &AtomicUsize,
    ...
    #[cfg(test)]
    pause_at_orderly_attach_eof_boundary(
        &BEFORE_ORDERLY_ATTACH_EOF_CONTROL_SEAL_PAUSE,
        live_input.identity,
    )
    .await;
    let terminal_control_pending = seal_orderly_attach_control_intake(
        attach_controls,
        deferred_controls,
        control_backlog,
        live_input.identity,
    ...
    // Receiver close is the EOF/control linearization boundary. Every control
    // send that won before it has now been drained into the existing deferred
    // queue, and every later send must fail. A terminal winner owns the next
    // outer pass even when its producer has not yet published `closing`.
    let close_owns_input = terminal_control_pending
        || matches!(socket_reentry, AttachSocketReentry::ClosingControl)
        || closing.load(Ordering::SeqCst);
    if close_owns_input {
        // The registration carrying these bytes is closing. Clearing both
        // forms of retained input is identity safety, not EOF truncation: no
        // later session may receive bytes accepted for this identity.
        pending_input.clear();
        pending_escape_flush.clear();
        *decoder = AttachFrameDecoder::new();
    }
    if !deferred_controls.is_empty() {
        return Ok(true);
    }
    if !close_owns_input && matches!(socket_reentry, AttachSocketReentry::InterFrameBarrier) {
        return Ok(true);
    }
    if close_owns_input {
        return Ok(false);
    ...
    if let Err(error) =
        flush_pending_attach_input_at_eof(live_input, pending_input, pending_escape_flush, locked)
            .await
    {
        // A close producer can lose the receiver-seal race and invalidate the
        // registration while the EOF-owned input flush is awaiting identity
        // work. Its terminal send is already proven unable to win, so stale
        // retained input is discarded and EOF completes cleanly.
        if closing.load(Ordering::SeqCst) {
            pending_input.clear();
            pending_escape_flush.clear();
            *decoder = AttachFrameDecoder::new();
    ...
    if !decoder.is_empty() {
        return Err(io::Error::new(
            ...
            "attach stream closed with an incomplete input frame",
        ));
    }
    Ok(false)
}
...
#[cfg(any(unix, windows))]
async fn seal_orderly_attach_control_intake(
    attach_controls: &mut Option<mpsc::UnboundedReceiver<AttachControl>>,
    deferred_controls: &mut VecDeque<AttachControl>,
    control_backlog: &AtomicUsize,
    ...
    let mut terminal_control_pending = deferred_controls.iter().any(is_terminal_attach_control);
    let Some(control_rx) = attach_controls.as_mut() else {
        return terminal_control_pending;
    };

    // `close` rejects sends that linearize later. Draining to `None`, rather
    // than stopping at a transient `try_recv(Empty)`, also waits for any
    // in-flight send that acquired its channel permit before this close.
    control_rx.close();
    #[cfg(test)]
    pause_at_orderly_attach_eof_boundary(&AFTER_ORDERLY_ATTACH_EOF_CONTROL_SEAL_PAUSE, identity)
        .await;
    while let Some(control) = control_rx.recv().await {
        attach_control::release_attach_control_backlog(
            control_backlog,
            control.received_backlog_units(),
        );
        terminal_control_pending |= is_terminal_attach_control(&control);
...
#[cfg(any(unix, windows))]
fn is_terminal_attach_control(control: &AttachControl) -> bool {
    matches!(
        control,
...
#[cfg(any(unix, windows))]
async fn flush_pending_attach_input_at_eof(
    live_input: &LiveAttachInputContext,
    pending_input: &mut Vec<u8>,
    pending_escape_flush: &mut PendingEscapeFlush,
    locked: bool,
    ...
    while !pending_input.is_empty() {
                ...
                .await
        {
            ...
            return Err(io::Error::other(
                "stale attach forwarder retained input at EOF",
            ));
        }
        if locked {
            pending_input.clear();
        ...
        live_input
            ...
            .await?;
        pending_escape_flush.clear();
        if !pending_input.is_empty() && pending_input.len() >= retained_before {
            return Err(io::Error::other(
                "attach EOF retained-input flush made no bounded progress",
            ));
        }
        // A flush can reroute its remainder through the normal input decoder
        // and retain one fresh ambiguous suffix. Strict length decrease bounds
        // the loop by the original retained byte count without discarding that
        // valid suffix or inventing a second input interpretation.
    }
    Ok(())
}
...
async fn process_attach_socket_messages(
...
) -> io::Result<AttachSocketReentry> {
    let processing = match process_socket_messages_with_status(
        decoder,
        stream,
        live_input,
        Some(current_target),
    ...
    .await
    {
        ...
        Err(_) if closing.load(Ordering::SeqCst) => {
            // Input may already be between the queue poll and its identity
            // check when close removes the registration. Discard that stale
            // input; orderly EOF will seal control intake and either drain a
            // terminal control that linearized earlier or reject one that
            // linearizes later.
            PendingAttachInputState::new(pending_input, pending_escape_flush).clear();
            // Validation failed before the decoder could consume the complete
            // frame. The closing identity can never apply it, and keeping it
            // would repeat the same stale validation forever when a sender
            // published `closing` without leaving a terminal control.
            *decoder = AttachFrameDecoder::new();
            return Ok(AttachSocketReentry::ClosingControl);
        }
        Err(error) => return Err(error),
    };
    if processing.forwarded_to_pane {
    ...
    sync_pending_escape_flush(pending_escape_flush, live_input, pending_input).await;
    Ok(
        if processing.reenter_outer_loop || pending_escape_deadline_due(pending_escape_flush) {
            AttachSocketReentry::InterFrameBarrier
        } else {
            AttachSocketReentry::None
        },
    )
}
...
struct SocketMessageProcessing {
    forwarded_to_pane: bool,
    reenter_outer_loop: bool,
}
...
async fn process_socket_messages(
    ...
    pending_input: PendingAttachInputState<'_>,
    active_emit_cache: &mut Option<(u64, rmux_proto::WindowTarget)>,
    locked: &mut bool,
) -> io::Result<bool> {
    Ok(process_socket_messages_with_status(
        decoder,
        stream,
        live_input,
        current_target,
...
async fn process_socket_messages_with_status(
    ...
    active_emit_cache: &mut Option<(u64, rmux_proto::WindowTarget)>,
    locked: &mut bool,
) -> io::Result<SocketMessageProcessing> {
    ...
    let mut reenter_outer_loop = false;
