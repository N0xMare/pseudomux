...
use super::{
    ...
    finish_pending_attach_exit, finish_pending_attach_exit_with_batch,
    flush_due_pending_escape_input, forward_attach,
    install_after_orderly_attach_eof_control_seal_pause,
    install_before_orderly_attach_eof_control_seal_pause, install_live_attach_input_apply_pause,
    install_live_attach_input_validation_pause,
    install_orderly_attach_eof_retained_input_validation_pause, is_predictable_local_echo,
    pane_output_channel, pane_output_channel_with_limits, pending_attach_exit_output_batch,
    predictable_local_echo_prefix_len, prepare_orderly_attach_eof, process_attach_data_payload,
    process_socket_messages, process_socket_messages_with_status, should_emit_overlay,
    sync_pending_escape_flush_with_escape_time, AttachControl, AttachControlSender,
    AttachSocketReentry, AttachTarget, LiveAttachInputContext, OverlayFrame, PredictedEcho,
};
...
#[derive(Default)]
struct PendingThenEofAttachIo {
    returned_pending: bool,
}

impl tokio::io::AsyncRead for PendingThenEofAttachIo {
    fn poll_read(
...
impl tokio::io::AsyncWrite for PendingThenEofAttachIo {
    fn poll_write(
...
async fn complete_input_frame_is_dispatched_before_orderly_eof() {
    ...
    assert_eq!(
        captured, input,
        "every complete frame read before EOF must mutate the pane before detach"
    );
}

#[tokio::test]
async fn complete_frame_after_unlock_barrier_is_dispatched_before_orderly_eof() {
    let input = b"y\r";
    let mut wire = encode_attach_message(&AttachMessage::Unlock).expect("encode unlock barrier");
    wire.extend_from_slice(
        &encode_attach_message(&AttachMessage::Data(input.to_vec()))
            .expect("encode input after unlock barrier"),
    );
    let (result, captured) =
        run_preclosed_attach_input("complete-input-after-unlock-at-eof", &wire).await;
    result.expect("attach drains every complete frame across the unlock barrier");
    assert_eq!(
        captured, input,
        "orderly EOF must not misclassify a complete frame deferred by an unlock barrier"
    );
}

#[tokio::test]
async fn selected_eof_reenters_after_unlock_barrier_before_classifying_residual() {
    let handler = Arc::new(RequestHandler::new());
    let attach_pid = 910_031;
    let target = create_attach_input_test_session(&handler, "selected-eof-unlock-barrier").await;
    let session_name = target.session_name().clone();

    // Keep the real registration's control queue separate. Unlock refreshes
    // that queue, while the forwarder gets an intentionally idle queue so its
    // selected socket-read arm is the only ready branch after the barrier.
    let (registered_control_tx, _registered_control_rx) = mpsc::unbounded_channel();
    handler
        .register_attach(attach_pid, session_name.clone(), registered_control_tx)
        .await;
    let (_forward_control_tx, forward_control_rx) = mpsc::unbounded_channel();
    let pane_output = pane_output_channel();
    let attach_target =
        test_attach_target_with_output(&session_name, b"BASE", None, pane_output.clone(), false);
    ...
    let result = tokio::time::timeout(
        ...
        forward_attach(
            AttachTransport::from_io(PendingThenEofAttachIo::default()),
            attach_target,
            wire,
            shutdown_rx,
            forward_control_rx,
    ...
    .await
    .expect("selected EOF path must terminate");
    result.expect("selected EOF must re-enter after a deliberate inter-frame barrier");
    assert_eq!(
        handler
            .attached_input_capture_for_test(&target)
            .await
            .expect("input capture remains installed"),
        input,
        "selected EOF must not classify the complete deferred frame as a truncated tail"
    );
}

#[tokio::test]
async fn truncated_input_frame_at_eof_fails_without_mutating_the_pane() {
    let (result, captured) = run_preclosed_attach_input("truncated-input-at-eof", &[1, 2, 0]).await;
    assert_eq!(
        result.expect_err("truncated input must fail closed").kind(),
        std::io::ErrorKind::UnexpectedEof
    );
    assert!(
        captured.is_empty(),
        "a partial frame must not reach the pane"
    );
}

#[tokio::test]
async fn complete_prefix_is_dispatched_once_before_truncated_tail_fails() {
    let complete = b"ok";
    let mut wire = encode_attach_message(&AttachMessage::Data(complete.to_vec()))
        .expect("encode complete prefix");
    wire.extend_from_slice(&[1, 5, 0]);
    let (result, captured) =
        run_preclosed_attach_input("complete-prefix-truncated-tail", &wire).await;
    assert_eq!(
        result.expect_err("truncated tail must fail closed").kind(),
        std::io::ErrorKind::UnexpectedEof
    );
    assert_eq!(
        captured, complete,
        "the complete prefix must dispatch once and the partial tail never mutates"
    );
}

#[tokio::test]
async fn final_complete_data_frame_flushes_newly_retained_escape_before_orderly_eof() {
    let input = b"A\x1b";
    let wire = encode_attach_message(&AttachMessage::Data(input.to_vec()))
        .expect("encode final retained attach input");
    let (result, captured) =
        run_preclosed_attach_input("final-retained-escape-at-eof", &wire).await;

    result.expect("a complete final frame with retained input must drain cleanly");
    assert_eq!(
        captured, input,
        "orderly EOF must force the final complete frame's ambiguous suffix through the normal input semantics"
    );
}

#[tokio::test]
async fn already_due_retained_escape_reenters_and_flushes_before_selected_eof() {
    let handler = Arc::new(RequestHandler::new());
    let attach_pid = 910_032;
    let target = create_attach_input_test_session(&handler, "due-retained-escape-at-eof").await;
    let session_name = target.session_name().clone();
    let escape_time = handler
        .handle(Request::SetOption(rmux_proto::SetOptionRequest {
            scope: ScopeSelector::Global,
            option: OptionName::EscapeTime,
            value: "0".to_owned(),
    ...
    // Prove the inter-frame disposition itself. A due retained ambiguity owns
    // one outer-loop pass even when the frame decoder has no residual bytes.
    let (probe_stream, _probe_peer) = tokio::net::UnixStream::pair().expect("probe stream pair");
    let probe_stream = AttachTransport::from(probe_stream);
    let mut probe_decoder = AttachFrameDecoder::new();
    let mut probe_pending = b"\x1b".to_vec();
    let mut probe_flush = PendingEscapeFlush::default();
    sync_pending_escape_flush_with_escape_time(&mut probe_flush, &probe_pending, Duration::ZERO);
    let mut active_emit_cache = None;
    let mut locked = false;
    let probe = process_socket_messages_with_status(
        &mut probe_decoder,
        &probe_stream,
        &live_input,
        None,
        PendingAttachInputState::new(&mut probe_pending, &mut probe_flush),
        &mut active_emit_cache,
        &mut locked,
    )
    .await
    .expect("due retained-input probe remains valid");
    assert!(
        probe.reenter_outer_loop,
        "a due retained ambiguity must re-enter even without decoder residual"
    );

    let wire = encode_attach_message(&AttachMessage::Data(b"\x1b".to_vec()))
        .expect("encode due retained input");
    let (_shutdown_tx, shutdown_rx) = watch::channel(());
    let result = tokio::time::timeout(
        Duration::from_secs(2),
        forward_attach(
    ...
    .await
    .expect("selected EOF with due retained input must terminate");
    result.expect("selected EOF must flush the due retained input cleanly");
    assert_eq!(
        handler
            .attached_input_capture_for_test(&target)
            .await
            .expect("input capture remains installed"),
        b"\x1b",
        "selected EOF must not discard an already-due retained escape"
    );
}
...
async fn assert_preclosed_session_exit_during_input_pause_drains_final_output(
    ...
    let handler = Arc::new(RequestHandler::new());
    let pane_target = create_attach_input_test_session(&handler, session).await;
    ...
    let attach_pid = if after_validation { 912_045 } else { 912_044 };
    let closing = Arc::new(AtomicBool::new(false));
    let (control_tx, control_rx) = mpsc::unbounded_channel();
    handler
        .register_attach_with_closing(
    ...
    let live_input =
        LiveAttachInputContext::current_for_test(Arc::clone(&handler), attach_pid).await;
    let pause = if after_validation {
        // Pause after the first successful identity validation but before its
        // Data mutation, so server close has already queued its terminal
        // control when the preclosed socket reaches the EOF seal.
        install_live_attach_input_apply_pause(live_input.identity)
    } else {
        install_live_attach_input_validation_pause(live_input.identity)
    };
    let pane_output = pane_output_channel();
    let (stream, mut peer) = tokio::net::UnixStream::pair().expect("attach stream pair");
    peer.shutdown()
        .await
        .expect("orderly-close attach input before forwarder starts");
    let initial_socket_bytes = encode_attach_message(&AttachMessage::Data(initial_data.to_vec()))
        .expect("encode initial racing Data frame");
    let (_shutdown_tx, shutdown_rx) = watch::channel(());
    let attach_task = tokio::spawn(forward_attach(
        stream,
    ...
    tokio::time::timeout(Duration::from_secs(2), pause.reached.notified())
        .await
        .expect("preclosed initial Data reaches the requested validation pause");
    let _ = pane_output.send_for_generation(None, final_output.to_vec());
    let _ = pane_output.send_for_generation(None, Vec::new());
    let killed = handler
        .handle(Request::KillSession(KillSessionRequest {
            target: session_name,
    ...
    let exited = read_attach_data_until(&mut peer, b"[exited]\r\n").await;
    let tail = exited
        .windows(final_output.len())
        .position(|window| window == final_output)
        .expect("final output must survive the preclosed concurrent input close");
    let banner = exited
        .windows(b"[exited]\r\n".len())
        .position(|window| window == b"[exited]\r\n")
        .expect("terminal control must retain exit ownership");
    assert!(tail < banner, "final output must precede the exit banner");
    assert!(
        attach_task.await.expect("attach task join").is_ok(),
        "terminal close must outrank orderly input EOF"
    );
}

#[tokio::test]
async fn preclosed_session_exit_before_input_validation_drains_final_output() {
    assert_preclosed_session_exit_during_input_pause_drains_final_output(
        "preclosed-exit-before-input-validation",
        false,
        b"RACING_INPUT",
        b"FINAL_BEFORE_VALIDATION_CLOSE",
    )
    .await;
}

#[tokio::test]
async fn preclosed_session_exit_after_input_validation_drains_final_output() {
    // Close publication lands after successful validation but before the
    // validated Data mutation and before the EOF/control seal.
    assert_preclosed_session_exit_during_input_pause_drains_final_output(
        "preclosed-exit-after-input-validation",
        true,
        b"\x1b",
        b"FINAL_AFTER_VALIDATION_CLOSE",
    )
    .await;
}

#[tokio::test]
async fn terminal_control_enqueued_before_closing_publication_wins_over_orderly_eof() {
    let handler = Arc::new(RequestHandler::new());
    let attach_pid = 912_048;
    let pane_target =
        create_attach_input_test_session(&handler, "eof-control-before-closing-publication").await;
    let session_name = pane_target.session_name().clone();
    let escape_time = handler
        .handle(Request::SetOption(rmux_proto::SetOptionRequest {
            scope: ScopeSelector::Global,
    ...
    let live_input =
        LiveAttachInputContext::current_for_test(Arc::clone(&handler), attach_pid).await;
    let pause = install_before_orderly_attach_eof_control_seal_pause(live_input.identity);
    let (raw_control_tx, control_rx) = mpsc::unbounded_channel();
    let control_backlog = Arc::new(AtomicUsize::new(0));
    ...
    let attach_task = tokio::spawn(forward_attach(
        ...
        test_attach_target_with_output(&session_name, b"BASE", None, pane_output.clone(), false),
        encode_attach_message(&AttachMessage::Data(b"\x1b".to_vec()))
            .expect("encode retained input before the EOF/control race"),
        shutdown_rx,
        control_rx,
        control_backlog,
        Arc::clone(&closing),
    ...
    tokio::time::timeout(Duration::from_secs(2), pause.reached.notified())
        .await
        .expect("orderly EOF reaches the control-intake boundary");
    let final_output = b"FINAL_BEFORE_CLOSING_PUBLICATION";
    let _ = pane_output.send_for_generation(None, final_output.to_vec());
    let _ = pane_output.send_for_generation(None, Vec::new());
    control_tx
        .send(AttachControl::Exited)
        .expect("enqueue terminal control before publishing closing");
    assert!(
        !closing.load(Ordering::SeqCst),
        "the regression must hold the producer in the queue-before-latch window"
    );
    pause.release.notify_one();
    ...
    let tail = exited
        .windows(final_output.len())
        .position(|window| window == final_output)
        .expect("final output must remain owned by the queued Exited control");
    let banner = exited
        .windows(b"[exited]\r\n".len())
        .position(|window| window == b"[exited]\r\n")
        .expect("queued Exited control must emit its terminal banner");
    assert!(tail < banner, "final output must precede the exit banner");
    assert!(
        attach_task.await.expect("attach task join").is_ok(),
        "the queued terminal control must win its EOF linearization"
    );
    assert!(
        handler
            ...
            .expect("input capture remains installed")
            .is_empty(),
        "the queued terminal winner must discard due input from its stale identity"
    );
}

#[tokio::test]
async fn orderly_eof_seal_rejects_terminal_enqueue_after_closing_publication() {
    let handler = Arc::new(RequestHandler::new());
    let attach_pid = 912_049;
    let pane_target =
        create_attach_input_test_session(&handler, "eof-closing-before-terminal-enqueue").await;
    let session_name = pane_target.session_name().clone();
    let closing = Arc::new(AtomicBool::new(false));
    let (registered_control_tx, _registered_control_rx) = mpsc::unbounded_channel();
    handler
    ...
    let live_input =
        LiveAttachInputContext::current_for_test(Arc::clone(&handler), attach_pid).await;
    let pause = install_after_orderly_attach_eof_control_seal_pause(live_input.identity);
    let (control_tx, control_rx) = mpsc::unbounded_channel();
    let control_backlog = Arc::new(AtomicUsize::new(0));
    let (_shutdown_tx, shutdown_rx) = watch::channel(());
    closing.store(true, Ordering::SeqCst);
    ...
    tokio::time::timeout(Duration::from_secs(2), pause.reached.notified())
        .await
        .expect("orderly EOF seals control intake after the closing publication");
    assert!(
        control_tx.send(AttachControl::Detach).is_err(),
        "a terminal enqueue linearized after the EOF seal must be rejected"
    );
    pause.release.notify_one();
    tokio::time::timeout(Duration::from_secs(2), attach_task)
        .await
        .expect("the sealed closing-without-control path must remain bounded")
        .expect("attach task join")
        .expect("EOF wins cleanly when the terminal enqueue linearizes later");
    assert!(
        handler
            .attached_input_capture_for_test(&pane_target)
            .await
            .expect("input capture remains installed")
            .is_empty(),
        "a losing close producer cannot make stale input observable"
    );
}

#[tokio::test]
async fn orderly_eof_seal_wins_close_during_retained_input_validation() {
    let handler = Arc::new(RequestHandler::new());
    let attach_pid = 912_050;
    let pane_target =
        create_attach_input_test_session(&handler, "eof-seal-before-retained-validation-close")
            .await;
    let session_name = pane_target.session_name().clone();
    let closing = Arc::new(AtomicBool::new(false));
    let (registered_control_tx, _registered_control_rx) = mpsc::unbounded_channel();
    ...
    let live_input =
        LiveAttachInputContext::current_for_test(Arc::clone(&handler), attach_pid).await;
    let pause = install_orderly_attach_eof_retained_input_validation_pause(live_input.identity);
    let (raw_control_tx, control_rx) = mpsc::unbounded_channel();
    let control_backlog = Arc::new(AtomicUsize::new(0));
    let control_tx = AttachControlSender::new(
        raw_control_tx,
    ...
    let attach_task = tokio::spawn(forward_attach(
        ...
        test_attach_target(&session_name, b"BASE", None),
        encode_attach_message(&AttachMessage::Data(b"\x1b".to_vec()))
            .expect("encode retained input for post-seal validation"),
        shutdown_rx,
        control_rx,
        control_backlog,
        Arc::clone(&closing),
    ...
    tokio::time::timeout(Duration::from_secs(2), pause.reached.notified())
        .await
        .expect("EOF seals control intake before retained-input validation");
    assert!(
        control_tx.send(AttachControl::Detach).is_err(),
        "a close producer that loses the EOF seal must observe a closed receiver"
    );
    assert!(
        closing.load(Ordering::SeqCst),
        "the losing sender must publish the shared closing state"
    );
    pause.release.notify_one();

    tokio::time::timeout(Duration::from_secs(2), attach_task)
        .await
        .expect("post-seal identity invalidation must remain bounded")
        .expect("attach task join")
        .expect("the EOF winner must absorb the losing close cleanly");
    assert!(
        handler
            .attached_input_capture_for_test(&pane_target)
            .await
            .expect("input capture remains installed")
            .is_empty(),
        "retained input for the invalidated identity must not reach the pane"
    );
}

#[tokio::test]
async fn preclosed_closing_without_terminal_control_is_bounded() {
    let handler = Arc::new(RequestHandler::new());
    let attach_pid = 912_046;
    let pane_target =
        create_attach_input_test_session(&handler, "preclosed-closing-without-control").await;
    let session_name = pane_target.session_name().clone();
    let closing = Arc::new(AtomicBool::new(false));
    let (control_tx, control_rx) = mpsc::unbounded_channel();
    handler
        ...
        .await;
    closing.store(true, Ordering::SeqCst);
    let wire = encode_attach_message(&AttachMessage::Data(b"STALE_AFTER_CLOSE".to_vec()))
        .expect("encode stale final input");
    let (_shutdown_tx, shutdown_rx) = watch::channel(());
    ...
    .await
    .expect("a closing latch without a queued control must not spin");
    result.expect("bounded closing fallback should detach cleanly at orderly EOF");
    assert!(
        handler
            .attached_input_capture_for_test(&pane_target)
            .await
            .expect("input capture remains installed")
            .is_empty(),
        "input accepted for a closing identity must never reach the pane"
    );
}

#[tokio::test]
async fn published_close_outranks_due_eof_barrier_before_pending_flush() {
    let handler = Arc::new(RequestHandler::new());
    let attach_pid = 912_047;
    let pane_target =
        create_attach_input_test_session(&handler, "published-close-before-due-flush").await;
    let session_name = pane_target.session_name().clone();
    let closing = Arc::new(AtomicBool::new(false));
    let (control_tx, control_rx) = mpsc::unbounded_channel();
    handler
    ...
    let stream = AttachTransport::from(stream);
    let mut decoder = AttachFrameDecoder::new();
    let mut pending_input = b"\x1b".to_vec();
    let mut pending_escape_flush = PendingEscapeFlush::default();
    sync_pending_escape_flush_with_escape_time(
        &mut pending_escape_flush,
        &pending_input,
    ...
    let _ = pane_output.send_for_generation(None, b"FINAL_CLOSE_BEFORE_DUE_FLUSH".to_vec());
    let _ = pane_output.send_for_generation(None, Vec::new());
    let killed = handler
        .handle(Request::KillSession(KillSessionRequest {
            target: session_name,
    ...
    assert!(
        prepare_orderly_attach_eof(
            &mut decoder,
            AttachSocketReentry::InterFrameBarrier,
            &live_input,
            &closing,
            &mut attach_controls,
            &mut deferred_controls,
        ...
        .await
        .expect("published close must be a valid EOF fence"),
        "the published terminal control owns one bounded outer-loop pass"
    );
    assert!(
        pending_input.is_empty() && pending_escape_flush.deadline().is_none(),
        "close publication must discard retained input with its now-stale identity"
    );
    ...
    .await
    .expect("the next outer pass cannot fail before polling the queued terminal control");

    let mut render_generation = 0;
    let mut overlay_generation = 0;
    let mut persistent_overlay = None;
    let mut persistent_overlay_visible = false;
    let mut persistent_overlay_state_id = current_target.persistent_overlay_state_id;
    let mut locked = false;
    let action = apply_pending_attach_controls(
    ...
    .await
    .expect("queued terminal control remains readable");
    let PendingAttachAction::Exit(exit) = action else {
        panic!("published Exited control must retain terminal ownership");
    };
    ...
    .await
    .expect("terminal output drain succeeds");
    let exited = read_attach_data_until(&mut peer, b"[exited]\r\n").await;
    let tail = exited
        .windows(b"FINAL_CLOSE_BEFORE_DUE_FLUSH".len())
        .position(|window| window == b"FINAL_CLOSE_BEFORE_DUE_FLUSH")
        .expect("final output must survive close publication");
    let banner = exited
        .windows(b"[exited]\r\n".len())
        .position(|window| window == b"[exited]\r\n")
        .expect("exit banner must be delivered");
