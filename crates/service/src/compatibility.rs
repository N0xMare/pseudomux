...
///   and `evidence/promoted-profile-2.1.258-macos-aarch64.json` (the floor's
///   own receipt). The 2.1.220 campaign receipt remains as history.
///
...
pub const PROMOTED_PROFILES: &[PromotedProfile] = &[
    PromotedProfile {
        ...
        drain_provenance: "POOLED conservative bound, not a per-version fit: max reachable \
                       post-answer transcript arrival 42 ms over 105 arrivals in minified-cell \
                       campaigns at Claude Code 2.1.258 (5 turns, max 42 ms) and 2.1.272 \
                       (100 turns, max 20 ms), x2.0 and rounded up to a 250 ms step = 250 ms. \
    ...
    PromotedProfile {
        ...
        terminal_profile: TerminalProfile::Transparent,
        input_transport: InputTransport::Sdk,
        transcript_drain_ms: 500,
        drain_provenance: "POOLED conservative bound, not a per-version fit: max reachable \
                       post-answer transcript arrival 165 ms over 150 arrivals in 102 \
                       linux/aarch64 transcripts spanning Claude Code 2.1.258/2.1.272, x2.0 and \
                       rounded up to a 250 ms step = 500 ms. The maximum is a post-answer \
                       type=attachment row, emitted between the answer and the turn_duration \
                       marker by 50 of the 50 minified campaign turns at 2.1.272 and by none of \
                       the 50 at 2.1.258; the per-version fits are 250 ms and 500 ms, published \
                       to be read and NOT shipped. Priced: the full drain binds on 0 of 100 cli \
                       turns (every turn carried a turn_duration marker). MEASURED IN A NATIVE \
                       linux/arm64 CONTAINER on a Darwin host, which is this arch's own silicon \
