...
* the published subcommand set and each one's `API` / `Ops` label come from
  `pmux --help`, cross-checked against the `Command` variants in
  `bin/pmux/src/cli.rs` so a stale binary cannot quietly agree with a stale
  README; session commands were deleted, not hidden;
...
They need a built workspace: `cargo build --workspace` (debug is enough;
a stale `target/release` is preferred if present and will fail if it
disagrees with the source). `tools/dev/check.sh` runs this file.
"""
...
def binary(name: str) -> pathlib.Path:
    ...
    `tools/dev/check.sh` sets `PMUX_DOCUMENTED_SURFACE_BIN_DIR` to the debug
    tree it just built. Without that, release is preferred over debug; a stale
    binary fails loudly rather than agreeing with a stale README.
    """

    override = os.environ.get("PMUX_DOCUMENTED_SURFACE_BIN_DIR")
    if override:
        candidate = pathlib.Path(override) / name
        if candidate.is_file():
            return candidate
        raise AssertionError(
            f"PMUX_DOCUMENTED_SURFACE_BIN_DIR={override} has no {name}"
        )
    candidates = [
...
def flatten(text: str) -> str:
    ...
    is the same flattening a line-break-tolerant scan needs -- a scan that a
    wrap can defeat reports agreement over the sites it happened to still find.
    """
...
class DocumentedSurfaceTest(unittest.TestCase):
    ...
    def test_the_readme_command_table_is_the_binarys_own_subcommand_list(self):
        ...
        self.assertEqual(
            hidden,
            set(),
            "the session CLI must not remain as hidden Command variants; it was "
            "deleted, not hidden",
        )
        self.assertEqual(
            declared,
    ...
    def test_the_readme_names_every_path_b_flag_the_daemon_offers(self):
        """`--pool-*` / `--messages-*` is the whole configuration surface.
        ...
        flags = set(re.findall(r"--(?:pool|messages)-[a-z0-9-]+", text))
        self.assertIn(
            "--pool-parent",
            flags,
        ...
        for flag in sorted(flags):
            with self.subTest(flag=flag):
                # Bounded, not `in`: `--pool-retain-dir` is a prefix of
                # `--pool-retain-directory`, so a substring test calls a
    ...
    def test_the_readme_quickstart_starts_the_daemon_that_ask_needs(self):
        ...
        required = set(re.findall(r"--(?:pool|messages)-[a-z0-9-]+", body.split("\n}")[0]))
        self.assertTrue(required, "the pool-disabled refusal names no flag to derive from")
        ...
        self.assertIsNone(
            re.search(r"--messages-bind(?![a-z0-9-])", first),
