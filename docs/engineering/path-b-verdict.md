...
| Evidence prune is O(corpus) on every teardown | `crates/service/src/pool/evidence.rs:209` | mechanism confirmed by reading: `prune` runs on every `retain_instance_transcripts`; one `spawn_blocking` in all of `crates/service`, in `attach.rs`. Timing not re-measured here |
