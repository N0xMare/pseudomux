...
**Status:** Phase 0 working specification; rows marked `OPEN` are release
blockers. No external compatibility or portability claim is made by this file.

`spec.md` is normative for product behavior. This document is normative for
test ownership, public-surface coverage, and release-gate commands. The local
...
- `tools/phase0` and Docker code may acquire, bind, and publish evidence. Their
  transcript or process interpretations cannot close a product matrix row.
- Every production defect found during validation receives a minimized tracked
  regression before or with the fix.
- Test counts are supporting metadata. Only this invariant-to-test map can
  close Gate A.
...
close their deterministic matrix obligations. Every `OPEN-*` is a release
blocker. `EXTERNAL` rows are not Gate A blockers but are mandatory for the
final delivery. `OUT-OF-SCOPE` must never hide an exposed or partially
...
the contract. For 18 rows — those riding on `crates/e2e/tests/full_stack.rs` —
the word "rerun" was wrong for a different reason: until 2026-07-26 that suite
had never executed even once. It has now executed unmodified and passed 8/8
against the real daemon, the private rmux sidecar, real PTYs, and the
deterministic fake Claude, so those 18 rows are `AUTHORED` on observed evidence
rather than on source inspection alone. What they still lack is execution
against a frozen release candidate and a published receipt. Section 4's row
structure is not restructured before the freeze; this paragraph is the
...
A live campaign re-observes the frozen candidate at six sites — every call to
`_verify_candidate_unchanged` in `tools/phase0/phase0_lib.py`, and
`grep -c 'self\._verify_candidate_unchanged(' tools/phase0/phase0_lib.py` must
print `6`. Each of those calls `observe_source_identity`, which is not a
...
the poller for the window, or clone elsewhere.
...
### The campaign `--cwd` must already be trusted

An untrusted directory produces a folder-trust screen that `blocking_screen`
classifies as `NeedsInputKind::Trust`
(`crates/service/src/driver_io.rs:1448-1454`), which the startup wait returns as
`TerminalScreenState::NeedsInput` (`crates/service/src/native.rs:1696-1697`),
stores as `initial_needs_input` (`:997-1003`), and surfaces to the caller from
`crates/service/src/v1/registry.rs:77` and `crates/service/src/v1/actor.rs:448`;
the wire code `needs_trust` comes from the mapping at
`crates/service/src/driver_io.rs:379-387`. pmux will never answer it, and must
not: `docs/spec.md:1109` classifies `needs_trust` as operator input requiring
explicit authorized human action outside the turn. Three of the four detached
reservations of 2026-07-28 died precisely here — session started, real Claude
launched, zero tokens, no prompt submitted, ordinal spent
(`evidence/README.md`, *Four detached reservations*). Trust the directory by hand
once, before the campaign: start Claude interactively in that exact `--cwd`
(`tools/phase0/phase0.py:112`) and answer the prompt. Trust is keyed by
directory, so the standalone clone required above is a new, untrusted directory
— satisfying the first precondition creates this one.
...
value the driver passes.

A permission prompt is terminal for an unattended run in the only sense that
matters: nobody will ever answer it. Select `--permission-mode`
(`tools/phase0/phase0.py:177-184`) so every grade that calls a tool proceeds
without a modal; note that `dangerously-skip-permissions` makes every turn
republish the `dangerous_permission_bypass` warning and is rejected for
`--scenario claude-p-one-shot` (`phase0_lib.py::validate_config`). The
...
The remedy is configuration, not code. Whatever is eventually done about the
permission case (`docs/current-state.md` section 9.9), the
`NeedsInputKind::Trust` path keeps its current behaviour: `docs/spec.md:1109`
requires human action outside the turn, and an automatic answer to a trust
prompt would be a security change, not a robustness fix.

### The driver must reserve against the tracked ledger

`tools/phase0` locates the ledger only by explicit `--ledger` path
...
`APPROVED_EFFORTS = ("low", "medium")` (`tools/phase0/phase0_lib.py`) is
enforced in `phase0_lib.py::validate_config` and mirrored in the CLI's
`--effort` choices (`tools/phase0/phase0.py:159`). A campaign at `high` is
outside the approved envelope and cannot produce promotable evidence, whatever
it observes.

## 3. Deterministic Gate A command manifest

Gate A is one ordered manifest. A command that is unavailable, skipped without
an applicable documented platform exclusion, flaky, or dependent on an
untracked oracle fails the gate.
...
fresh release build, captures all eight required executable receipts and
...
python3 -m ruff check --no-cache clients/python tools/evidence_common tools/package-smoke tools/phase0 tools/linux-docker tools/gate-a tools/gate-a-candidate
python3 -m ruff format --check --no-cache clients/python tools/evidence_common tools/package-smoke tools/phase0 tools/linux-docker tools/gate-a tools/gate-a-candidate
```
...
Actual package artifacts are part of the gate. Each command builds one exact
artifact under an identity-fenced temporary root with credentials, indexes,
scripts, audits, and dependency resolution disabled; validates and hashes the
archive; installs only that artifact into an isolated consumer; imports the
documented native API and type metadata; and proves exact cleanup without
publishing or changing either client tree:
...
The following exact invocations are normative. The tracked targets exist;
Gate A remains open until every invocation runs with its bounded reproducible
seed/corpus on the unchanged candidate:
...
```bash
# The candidate prelude already built and froze the exact eight binaries.
...
# The cross-cell contamination sweep is in the same crate and runs above at
# N=2/5/15 against the double, together with its own discrimination self-test.
# Lane (b) skips loudly unless it is asked for, because it spends real model
# calls against the operator's own credentials:
PMUX_E2E_BIN_DIR="$PMUX_GATE_A_RELEASE_DIR" PMUX_CONTAMINATION_REAL_CLAUDE=1 \
  cargo +1.88 test --locked -p pseudomux-e2e --test cross_cell_contamination \
  -- --ignored --nocapture real_claude
...
```bash
PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover -s tools/evidence_common/tests -v
...
PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover -s tools/gate-a-candidate/tests -v
PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover -s tools/gate-a/tests -v
bash -n scripts/gate-a-fuzz.sh scripts/gate-a-mutants.sh scripts/gate-a-residue.sh scripts/gate-in-worktree.sh scripts/path-b-done.sh scripts/pmuxd-run.sh tools/linux-docker/inside.sh tools/linux-docker/run.sh tools/linux-docker/suite.sh tools/screen-corpus/per_binary_tests.sh
shellcheck scripts/gate-a-fuzz.sh scripts/gate-a-mutants.sh scripts/gate-a-residue.sh scripts/gate-in-worktree.sh scripts/path-b-done.sh scripts/pmuxd-run.sh tools/linux-docker/inside.sh tools/linux-docker/run.sh tools/linux-docker/suite.sh tools/screen-corpus/per_binary_tests.sh
...
Shared evidence-common tests own the bounded finite-command and managed-process
supervision receipts used by every evidence consumer. Package-smoke tests own
archive, acquisition-environment, installed-artifact, and exact-cleanup
mechanics only. Phase-0 semantic parser tests cannot close product rows.
Candidate-envelope and Docker/tooling tests own source/tool/binary identity,
command causality, consent, accounting, redaction, artifact publication,
scoped cleanup, and evidence binding only. `tools/gate-a/tests` (65 tests, ~15 s,
measured 2026-08-12) owns three things and never invokes a real gate cell or
cargo: `test_run_gate.py` owns the Gate A runner itself — manifest parsing, cell
ordering, release-candidate preconditions, and receipt shape — over synthetic
cells only; `test_documented_surface.py` owns `README.md`'s documented
surface, deriving it from `pmux --help`, `pmuxd serve --help`, `pmux-mcp`'s own
`tools/list`, `MAX_POOL_SIZE` and `MODEL_TABLE`, so an undocumented subcommand,
model or Path B flag is a red cell rather than a reader's problem; and
`test_pinned_worktree.py` owns where `scripts/gate-in-worktree.sh` puts its
receipt and whether the files that receipt hashes outlive the work directory,
over a one-commit repository it builds under `target/`. It runs those
binaries with `--help` and on stdin; it starts no daemon and spends no tokens.
Every command
in the block above has a `gate_f` cell in
`tools/gate-a-candidate/phase-manifest.json`, and
`test_gate_f_runs_every_unittest_directory_the_doc_requires` derives that
correspondence from this block rather than restating it: the
`tools/linux-docker/tests` line was the one that had no cell, and nothing
compared the two.

### The SIGKILL crash harness, and why it is not a gate cell

`tools/crash-harness` is the instrument that measured `docs/spec.md` §4.8.2's
crash-safety claims for the agent store. It is **detached from the workspace**,
with its own `[workspace]` table and its own lock, and that is deliberate:
`tools/screen-corpus/per_binary_tests.sh` enumerates every workspace target from
`cargo metadata` and prints *"every one of the N test targets passed in
isolation"*. As workspace members these two binaries would have widened that N
by five while contributing zero test cases — a count whose scope is wider than
its sentence, which is the defect §9.21 records.

```bash
cd tools/crash-harness && cargo build
H=./target/debug/crash-harness; C=./target/debug/crash-child
$H update  45 $C   # a looping updater, killed at a jittered offset
$H create  40 $C   # a looping creator, same
$H restart 40 $C   # one store, N crash-and-restart cycles
```

It is not a gate cell because it is a probabilistic instrument: `update` mode
measures how often a crash lands in a window, and a window nothing lands in is
reported as a clean run rather than as a gap. The durable protection for what it
found lives in `crates/service/tests/agent_resource.rs`, deterministic and in
the suite — `a_version_published_before_the_pointer_moved_is_adopted_and_never_wedges_the_agent`,
`a_head_pointer_that_regressed_resolves_forward_over_every_published_version`,
and `a_taken_version_name_that_is_not_a_readable_version_is_reported_and_never_published_over`.
The harness is what establishes that the states those tests construct by hand are
states a real crash reaches.

**Read its calibration pass before trusting a green run.** `update` mode measures
one update cycle on the host (≈20 ms here) and spreads the kill offset uniformly
across six of them. Its first version took the offset from
`subsec_nanos() % 4000`, sampled one phase of the cycle, and found **zero**
wedges in 40 trials against a store that wedged 42% of the time. A run whose
`landed in the window` count is zero has measured nothing, whatever its
`broken` count says.

Every mode removes its own work root, which lives under `TMPDIR` and carries the
harness pid — never under `/tmp`, so a killed harness cannot leave a directory
matching a prefix `scripts/gate-a-residue.sh` scans for.
...
| --- | --- | --- | --- | --- |
| PKG-01 | The actual TypeScript tarball and Python wheel are created without publication, registry/index acquisition, inherited credentials, install scripts, or dependency resolution; each archive is structurally validated under per-entry and cumulative decompression bounds, content-hashed, identity-fenced through installation into an isolated consumer, and proves the documented native API, runtime entrypoints, declarations or `py.typed`, metadata, complete bound npm/TypeScript or Python build-tool closure, unchanged client trees, bounded descendant cleanup, and identity-fenced temporary cleanup | L4 | `tools/package-smoke/package_smoke.py`, installed-artifact fixtures, and `tools/package-smoke/tests/test_package_smoke.py`; the materialized npm/TypeScript/Node-types/undici-types and Python runtime/build-support flows plus hostile closure, mutate/restore, archive-bomb, installed-tree, bounded-process, and cleanup regressions pass at source level; exact candidate-supplied closure execution remains pending | OPEN-L4 |
...
| P-08 | Reserved disconnect/heartbeat lease values fail as `unsupported_feature`; the default continue/no-heartbeat value is the only admitted v1 wire behavior | L0/L3/L4 | `crates/protocol/tests/v1_golden.rs::reserved_turn_lease_vectors_decode_as_valid_requests`; shared Rust/TS/Python reserved-lease tests; `crates/service/src/native.rs::tests::unimplemented_disconnect_leases_fail_closed`; `crates/e2e/tests/full_stack.rs::all_v1_methods_use_the_real_public_and_private_process_boundaries`; exact release Gate D rerun pending | OPEN-L3 |
...
| T-11 | Final text/blocks/tools/model/usage/provenance come only from terminal main logical message | L0/L2/L3 | `crates/service/tests/v1_actor.rs::actor_maps_fragmented_fixture_without_prior_sidechain_team_or_meta_leaks`; `crates/e2e/tests/full_stack.rs::{all_v1_methods_use_the_real_public_and_private_process_boundaries,assert_rich_result}`; exact release Gate D rerun pending | OPEN-L3 |

### 4.3 Launch, session actor, lifecycle, and security

| ID | Contract | Layer | Authoritative tracked location | Status |
| --- | --- | --- | --- | --- |
| S-01 | Interactive-only argv; forced new/resume identity; full forbidden flags and bounded raw allowlist | L0/L2/L3 | `crates/service/src/claude_launch.rs::{subscription_launch_is_interactive_and_sanitized,resume_uses_resume_instead_of_session_id,print_and_positional_passthrough_are_rejected,every_typed_launch_option_has_one_unambiguous_argv_mapping}`; fake-Claude child attestation in full-stack E2E; exact release Gate D rerun pending | OPEN-L3 |
| S-02 | Canonical paths, typed options, environment snapshot/patch, subscription stripping, transparent profile | L0/L2/L3 | canonical path/environment/profile tests in `crates/service/src/claude_launch.rs`; `crates/e2e/tests/full_stack.rs::{Sandbox::assert_common_launch,Sandbox::assert_rich_launch}`; exact release Gate D rerun pending | OPEN-L3 |
...
| S-07 | Session+generation fence; stale operations never retarget; exact close tombstone replay | L0/L2/L3 | `crates/service/tests/v1_actor.rs::stale_generation_operations_cannot_target_a_resumed_process`; tombstone unit tests; resume/tombstone composition in `crates/e2e/tests/full_stack.rs::all_v1_methods_use_the_real_public_and_private_process_boundaries`; exact release Gate D rerun pending | OPEN-L3 |
...
| S-10 | Stable cursor-correlated editor; one paste; changed stable render; at-most-one Enter; ambiguity reaps | L0/L2/L3 | admission/fence/ambiguity tests in `crates/service/src/driver_io.rs`; real ambiguous-paste cell in `crates/e2e/tests/full_stack.rs::all_v1_methods_use_the_real_public_and_private_process_boundaries`; exact release Gate D rerun pending | OPEN-L3 |
| S-11 | Typed needs-input; no automatic answers; admission modal fail/reap; post-Enter resumability | L0/L2/L3 | modal classifier/admission tests in `driver_io.rs`; actor modal-phase tests in `v1_actor.rs`; startup, admission, and post-Enter full-stack cells; exact release Gate D rerun pending | OPEN-L3 |
| S-12 | Cancellation sends one Ctrl-C; ready+quiet+post-interrupt drain or taint; next-turn isolation | L0/L2/L3 | `crates/service/tests/v1_actor.rs::{cancellation_recovers_or_taints_and_confirmed_close_terminates_the_actor,cancellation_requires_post_interrupt_transcript_stability,cancellation_drains_late_rows_before_an_immediate_fresh_turn}`; cancel-then-next-turn full-stack cell; exact release Gate D rerun pending | OPEN-L3 |
| S-13 | Attach one-use reservation/capability, mode/token/expiry/proxy, detach reconciliation and stale fence; complete client frames immediately followed by EOF are delivered exactly once without violating Unlock/control/output barriers, and terminal-control versus orderly-EOF races linearize at receiver close plus complete drain | L0/L2/L3 | `crates/service/src/attach.rs::{proxy_preserves_initial_output_and_both_stream_directions,closed_output_peer_cannot_cancel_already_authenticated_input,private_output_eof_releases_proxy_with_public_input_still_open}`; attach reservation/reconciliation tests in `crates/service/tests/v1_actor.rs`; every one of the fourteen patch-owned regressions in `vendor/rmux-server/src/pane_io/tests.rs` -- named there and in `vendor/rmux-server/PMUX-PATCH.md` and, since the `--exact` cells collapsed into one module filter, nowhere else -- including the three receiver-seal races, plus the targeted mutation proofs recorded in `vendor/rmux-server/PMUX-PATCH.md`; `bin/pmux-rmuxd/tests/process_blackbox.rs::real_attach_half_close_delivers_the_final_complete_frame_exactly_once`; real proxy and post-Enter attach cells in full-stack E2E; exact release Gate D rerun pending | OPEN-L3 |
| S-14 | Close retries until exact process boundary reaped; descendant escape invalidates proof | L0/L2/L3/L5 | `crates/service/tests/v1_actor.rs::unconfirmed_close_stays_retryable_until_process_reaping_is_confirmed`; `crates/service/tests/lifecycle_faults.rs::observed_descendant_escape_keeps_real_close_unconfirmed_across_retry`; `crates/e2e/tests/full_stack.rs::public_close_retry_never_claims_an_observed_escaped_descendant_was_reaped`; exact release Gate D/E reruns pending | OPEN-L5 |
...
| S-27 | The minified (Path B) cell is a start-time property admitted only on a tested compatibility profile, and it is refused before a Claude process is spawned; the same one rule also refuses at registration, so the `pub` registry is not a second door. `clear_session` is fenced on the transcript the caller believes is bound: matching rotates and reports `rotated: true`, and EVERY stale value — including one stale by exactly one rotation — is a non-retryable `id_conflict` carrying `stale_transcript_fence` that types nothing. There is no already-cleared answer, because a fence one rotation behind is byte-identical to the fence a session starts with, which is what a restarted or second caller holds; recovery from a lost response is reading `SessionSnapshot::transcript_session_id` back. The refusal is Step 0, ahead of the busy guard, and holds mid-turn and under a modal | L0/L3 | `crates/service/tests/minified_cell.rs::{a_session_registered_as_a_minified_cell_can_be_cleared_without_selection,an_untested_profile_cannot_register_a_minified_cell_at_all,a_clear_fenced_one_rotation_behind_is_refused_rather_than_answered,a_clear_fenced_two_rotations_behind_is_refused,a_stale_fence_is_refused_without_disturbing_a_turn_in_flight,a_stale_fence_cannot_claim_a_transcript_that_has_since_served_a_turn,a_stale_fence_is_refused_while_the_session_waits_on_a_modal,an_untested_compatibility_cell_cannot_select_the_minified_cell}`; one shared predicate at `crates/service/src/v1/actor.rs::require_tested_for_minified_cell` called from `native.rs::start_session_internal`, `registry.rs::register`, and `actor.rs::select_minified_cell` (the copy in `actor.rs::spawn` was DELETED as unreachable: `register` is the only caller of `spawn_actor` and applies the rule immediately before it); end-to-end over the real socket at `crates/e2e/tests/full_stack.rs::path_b_drives_a_minified_cell_through_clear_over_the_real_socket` | COVERED |
...
| S-36 | The root and cwd rules are properties of the INCUMBENT and of the RESOLVED resource, not of the applicant's request shape — and the relation they decide is CONTAINMENT, not identity. The invariant, stated exactly: **no directory a live minified cell binds may be reachable by any other session, in any role, at any depth** — as a configuration root, as a cwd, as an isolation root, as an ancestor of any of those, or through the `HOME` the delivered root is derived from. LEAK 7 was the gap between the message this rule already printed ("no other session may be launched **under it**") and the predicate it decided with (`must_treat_as_same_directory`, an IDENTITY test): `R/sub` is not `R`, so no incumbent was found and `admit_config_root` returned `SeedDisposition::Write` against a live cell's private root. EIGHT shapes were MEASURED as ADMITTED over the real socket against a live minified cell — config root nested in the cell root (absent subdir, and the cell's own `projects/`), config root an ANCESTOR of the cell root, `HOME` redirected so the delivered root landed at `<cell root>/.claude`, cwd IS the cell's configuration root, cwd inside it, cwd inside the cell's workspace, and a minified applicant's own canonicalized, owner-checked, pristine private root nested INSIDE the victim's — and the victim's own root ended up holding the intruder's transcript, `.claude.json` and `settings.json`. The rule is now asked with the containment predicate that already existed (S-39), inode-keyed and in both directions, over the full cross-product of every directory the applicant binds against every directory every live claim binds (`LiveResourceClaim::directories`). SYMMETRIC IN THE CELL: a live MINIFIED claim answers on containment whatever the applicant is, and a MINIFIED APPLICANT gets containment against every live claim including ordinary ones, because a private root nested inside a live ordinary session's workspace is the same leak one second later. ORDINARY-versus-ORDINARY stays IDENTITY and role-matched, deliberately: nesting is the ordinary shape of a filesystem, and widening that arm would refuse a second ordinary session working in a subdirectory AND — through the seed disposition — stop pmux seeding a private root that merely sits under a live session's cwd. Resources are still compared as directories rather than byte strings, so every leak-5 spelling remains one directory; where several sessions reach one resource, the strictest claim answers. Admission is keyed on `effective_config_root`, and the assumption that this covers an isolated start's NAMED root (step 6 of `build_environment` overwrites `CLAUDE_CONFIG_DIR` with the canonicalized isolation root) is now a CHECK rather than an assumption | L0 | `crates/service/src/native.rs::tests::{no_directory_a_live_minified_cell_binds_is_reachable_at_any_depth_in_any_role,containment_binds_a_minified_applicant_to_ordinary_sessions_and_leaves_them_to_each_other,an_ancestry_walk_is_bounded_and_refuses_a_loop_an_unreadable_ancestor_and_an_overlong_name,the_named_isolation_root_and_the_delivered_configuration_root_must_be_one_directory,a_root_a_live_minified_cell_holds_admits_no_other_session_in_any_shape,a_minified_cell_is_refused_a_root_any_live_session_is_already_using,a_minified_cell_is_refused_a_root_anything_has_ever_run_in,a_cwd_is_not_shared_with_a_minified_cell_in_either_direction,a_live_minified_cells_resources_are_refused_under_every_alias_of_the_same_inode,a_root_that_does_not_exist_yet_is_admitted_and_one_that_cannot_be_inspected_is_not,a_published_session_claims_the_root_and_cwd_of_its_own_transcript}` (the last builds a published `SessionMetadata` and drives the whole chain metadata → `live_resource_claims` → incumbent → refusal). The eight-relation test asserts BOTH applicant cells per row and carries a negative control, so it cannot pass by refusing everything. Rules at `native.rs::{admit_bound_resources,require_isolation_root_is_the_effective_root,claim_reaches,incumbent_cell_for,admit_config_root,admit_cwd,strictest_cell}`, containment from `claude_launch::one_directory_contains_the_other` (ONE implementation, two callers), resource identity from `native.rs::effective_config_root` and the resolved launch cwd. COST, MEASURED: an ancestry walk per resource per live cell is O(depth × cells) — 3.41 ms per admission against 15 live minified cells with a 14-component applicant path, against 3.17 µs with no live cells and 1.40 ms for a bare `/usr/bin/true` spawn on the same host. It is not on a latency path that cares: `admit_bound_resources` has exactly one production call site (`start_session_internal`), runs once per session START and never per turn or per `/clear`, and the same function then spawns Node for `detect_claude_version`. RESIDUAL, NARROWED BY S-48: that one call site still needs a live `NativeService` and is still only EXECUTED by `#[ignore]`d tests. What is no longer unwatched is its presence and its position — S-48 pins the four calls of the funnel prefix, in order, against `start_session_owned_with_retention`'s own source, so removing `admit_bound_resources` from the funnel or reordering it past the resolution it judges is a default-suite failure rather than a clippy warning. Deleting it outright is still a compile error (the seed disposition it binds is consumed downstream) | COVERED |
| S-37 | Cross-cell contamination is a STANDING sweep, not a scenario: N minified cells, each given a distinct unguessable secret, run concurrently against one daemon, are cleared, and then every byte reachable from each cell is searched for every OTHER cell's secret, session id, transcript id and cwd. The channels are a table (`ROOT_CHANNELS`) so adding one is a one-line row, every named channel is REPORTED present-or-absent so an unexercised channel cannot masquerade as a clean one, and anything a real Claude writes that no row claims is swept anyway and reported BY NAME under `unclassified-root-entry` (that report is what named the `cache/`, `sessions/`, `.credentials.json` and `__store.db` rows). Non-file channels asserted by the same needle sweep: the model's own answer to a recall prompt after `/clear`, the transcript inventory by FILE NAME (an empty foreign session file passes a content scan), the post-clear bound transcript under the stricter rule that it holds NOTHING including its own secret, and the operator's real `~/.claude` and `~/.claude.json` (byte-identity, plus leak assertions that are never downgraded). Anti-vacuity is explicit: a channel counts as tested only once the cell's OWN secret has been shown findable in it (`positive_controls`), which is why the `history.jsonl` assertion is required of the real lane and not of the double | L5 | `crates/e2e/tests/cross_cell_contamination.rs`. Lane (a), deterministic and credential-free against `pmux-test-claude`, at N=2/5/15: `{two,five,fifteen}_minified_cells_share_nothing_through_any_channel`. Lane (b), the operator's real `claude` behind `PMUX_CONTAMINATION_REAL_CLAUDE=1`: `real_claude_cells_share_nothing_through_any_channel` — the double cannot reproduce the TUI composer, `history.jsonl` recall or the paste cache, which is exactly how the composer blocker escaped four double-only rounds. DISCRIMINATION: `the_harness_reports_contamination_when_two_cells_share_one_configuration_root` runs the identical body with one root and one cwd shared by two FULL cells (the honest bypass — a minified pair is refused at admission by S-36, which would prove the guard rather than the sweep) and asserts the sweep reports it; measured 12 violations across `projects`, `transcript-inventory` and `global-config`. SECOND DOOR: every run also probes, while every cell is LIVE, the request shape the harness previously could not construct — an ORDINARY cell, no `config_isolation`, and the victim's root handed over in a plain `environment.set["CLAUDE_CONFIG_DIR"]` — under every spelling of it (identity, trailing slash, doubled separator, `.`, `..` through an existing directory, `..` through a MISSING one, and on macOS the firmlink alias). Each must be refused with `InvalidConfig`; an admission is a violation on the `plain-env-config-dir` channel AND is made to leave its on-disk trace, because the admitted intruder is given one turn so the transcript inventory catches its file inside the victim's root independently. `assert_clean` asserts the probe COUNT, not merely the absence of violations, so a probe that stopped constructing the shape cannot report the same clean sweep as one that constructed it and was refused. Because the live lanes are `#[ignore]`d, the default suite watches the table instead: `the_plain_env_spellings_include_the_one_the_kernel_reports_as_absent` asserts the leak-5b row is present, that every resolvable row is the same `(dev, ino)` as the root it aliases, and that the one absent row is completed onto that very inode by a recursive create. CONTAINMENT DOOR (LEAK 7): the second door varies the SPELLING of one directory; this one varies the RELATION. `probe_the_containment_door` runs, while every cell is LIVE, eight starts that never NAME a directory the victim binds and bind one anyway — a config root nested in the cell root (absent subdir, and the cell's own `projects/`), a config root that is an ANCESTOR of it, `HOME` redirected so the delivered root lands at `<cell root>/.claude`, a cwd that IS the cell's configuration root, a cwd inside it, a cwd inside the cell's workspace, and a MINIFIED applicant's own canonicalized, owner-checked, pristine private root nested inside the victim's. Each must be refused with `InvalidConfig`; an admission is a violation on the `live-cell-containment` channel and is given one turn so its transcript, `.claude.json` and `settings.json` land inside the victim's root where the sweep catches them independently. MEASURED before the containment rule: seven ADMITTED, the `HOME` row past the guard and killed downstream, nine violations swept. MEASURED after: eight refused, zero violations. `assert_clean` asserts this probe's COUNT as well. The default-suite share of it is `every_containment_relation_really_overlaps_a_directory_the_victim_binds`, which asserts on the TABLE with an inode-keyed ancestry check written INDEPENDENTLY of the service's: every row really overlaps a directory the victim binds, exactly one row is the same directory in the other role (so the table has not decayed into a spelling table — leak 5 already owns the equal-directory axis), all four entry doors and all three relations are represented, and exactly two rows bind a directory that does not exist yet — the case no identity test can see even in principle | COVERED |
| S-38 | A DIRECTORY pmux binds a session to may only be named in a spelling whose meaning does not depend on what exists. `..` is the one such construct: the kernel resolves left-to-right, so `/X/NOPE/../rootA` reports `NotFound` while `NOPE` is missing even though a recursive create — which is what Claude does to its own `CLAUDE_CONFIG_DIR`, and what `mkdir -p` does — creates the intermediate and lands the path on the live `/X/rootA`. MEASURED over the real socket before this: `--env CLAUDE_CONFIG_DIR=/X/NOPE/../rootA` was ADMITTED against a live minified cell holding `/X/rootA`, and the intruder's child wrote its own transcript physically inside that cell's root (LEAK 5b). Two independent refusals: the effective configuration root — for every shape that produces one, `set`, snapshot, or `HOME`-derived — must carry no `..` component at all; and the admission gate stops reading an absence as evidence for any `..` spelling of EITHER bound resource. pmux refuses rather than collapsing `..` lexically, because lexical collapsing is not the kernel's rule when a component is a symlink (`a/b/..` is `b`'s target's parent) and a wrong "different" is the leak while a wrong "same" is a refusal. The identity predicate itself keeps reporting the kernel's answer, because its other caller compares the securestorage PIN, which is a keychain-service input rather than a directory pmux binds | L0 | `crates/service/src/native.rs::tests::{an_effective_config_root_spelled_with_a_parent_component_is_refused,a_dot_dot_through_a_missing_directory_is_refused_as_either_bound_resource}` (the second asserts BOTH resources, both cells, and an EMPTY claim list — with no incumbent the old rule fell through to `SeedDisposition::Write`); the two filesystem facts are pinned as premises by `crates/service/src/claude_launch.rs::tests::an_absence_reported_for_a_dot_dot_spelling_proves_nothing` and the predicate's exact scope by `::only_a_parent_component_makes_a_spelling_depend_on_the_filesystem`. Rules at `native.rs::{effective_config_root,require_establishable_identity}` and `claude_launch.rs::traverses_a_parent_component`. Over the socket: `crates/e2e/tests/cross_cell_contamination.rs::probe_the_second_door` | COVERED |
...
| S-50 | PATH B RETAINS ITS OWN DRAIN EVIDENCE, AND NONE OF THE CONTENT. 178 of the 186 reachable arrivals behind the shipped drain came out of pmux's own PAID campaign directories, and a new Claude Code version has no `cli` turns to re-analyse -- so every destroyed pool instance's transcripts are now mirrored before `erase_tree`, into `path-b-evidence/` beside the socket, pruned to `evidence::RETAINED_ROW_FIELDS`. That set is DERIVED, not judged: it is exactly `measure_transcript_drain.py`'s `FIELDS_READ`, which the tool now PRUNES every row to on the way in, so the constant is load-bearing in the tool rather than decorative and nothing downstream can read a prompt even by accident. MEASURED: mirroring the 189 transcripts behind the 2.1.220 receipt produced 271,497 bytes and reproduced that receipt's `post_answer_arrivals`, `recommended_transcript_drain_ms`, `full_drain_binds_on` and 456-turn count IDENTICALLY. On by default, bounded at 64 MiB with oldest-first pruning by mtime, off with `--path-b-no-evidence`, and the running daemon publishes the directory it is using | L0/L1 | `crates/service/src/pool/evidence.rs::tests::{the_retained_fields_are_the_ones_the_measurement_tool_reads,a_mirror_keeps_the_measurement_and_none_of_the_content,a_row_that_is_only_content_is_dropped_entirely,the_retained_tree_never_exceeds_its_budget,an_instance_with_no_transcript_leaves_no_directory_behind}`; `crates/service/tests/path_b_pool.rs::ordinary_path_b_traffic_retains_its_own_drain_evidence_and_no_content`; `bin/pmuxd/src/main.rs::an_operator_may_state_a_version_range_and_overlapping_ones_are_refused` | COVERED |
| S-51 | THE MINIFIED LAUNCH BUNDLE IS PUBLISHED AS DATA AND CHECKED AGAINST THE ARGV A MINT EMITS. Three source files stated the flags a Path B cell launches with -- `v1/minified.rs`'s module doc, `measure_transcript_drain.py`'s `ROW_KINDS` premise and `tools/phase0/README.md` -- and all three named `--strict-mcp-config` and `--safe-mode`, neither of which any launch path emitted. MEASURED at 2.1.226 from the child's own `--debug-file`: without `--strict-mcp-config` a pristine minified cell in an empty private root fetches the OPERATOR'S ACCOUNT connector list over HTTP (`[claudeai-mcp] Fetching from https://api.anthropic.com/v1/mcp_servers`, plus a 294-entry registry load, 6 MCP lines); with it, 2 lines and `MCP configs resolved in 0ms`, `state: ready` on both arms. The flag is now driver-owned argv for `cell: minified` and for no other cell (`claude_launch::MINIFIED_CELL_FLAGS`), and `--safe-mode` is still not passed -- nothing it closes is measured open and it moves the TUI rendering every Path B screen constant was calibrated without. The two remaining prose sites publish the bundle as an ordered list that a test parses and compares, element for element in argv order, against the argv `stateless::launch_request_for` produces through the same three steps `start_session` drives it through; a workspace scan refuses any other code-tree file the right to name one of the spellings | L0/L1 | `crates/service/src/stateless.rs::tests::the_documented_minified_launch_bundle_is_the_argv_a_mint_emits`; `crates/service/src/claude_launch.rs::tests::a_minified_cell_is_launched_with_out_of_root_mcp_configuration_suppressed` | COVERED |
| S-52 | STOPPING A STARTING DAEMON IS A GRACEFUL SHUTDOWN, AND A FAILED START OWNS WHAT IT MINTED. `shutdown_signal()` was an `async fn` passed in argument position to `serve_until`, so `signal(SignalKind::terminate())` ran only when that future was first polled -- AFTER `NativeService::start` had minted the whole declared warm set. MEASURED against real Claude 2.1.226, `--path-b-warm claude-sonnet-5/low=3`, SIGTERM 2.6 s in: **exit 143**, three epoch trees left, a stale socket, and a daemon log holding ONE line -- the raw startup `writeln!` -- because the `tracing_appender::non_blocking` `WorkerGuard` never dropped. The handlers are now installed before anything is minted (exit 0, no tree, socket removed, `pmuxd protocol v1 listening` and `pmuxd stopped` both in the log), and the signal is OBSERVED rather than acted on early: a mid-mint SIGTERM finishes the declared warm set first, MEASURED at 4,560 ms, because main holds no service handle to abandon safely. The recovery chain was the half that was uncharacterised and it was EXPONENTIAL: a failed start erased the one tree it collided with and abandoned every tree it had minted before it, so `L -> (L \ {min L}) union {0..min L - 1}` -- every transition observed -- and three abandoned trees took **seven** refusing restarts, `2^w - 1`. `NativeService::start` now drains on a `start_pool` failure, making it one refusing restart per leftover tree (3 planted -> 3, MEASURED), and the refusal names the situation, the rule and what it did about the tree rather than the rule alone | L0/L3 | `crates/e2e/tests/pool_concurrency.rs::a_sigterm_during_the_warm_mint_shuts_down_gracefully_and_leaves_no_tree`; `crates/service/tests/path_b_pool.rs::{a_refused_epoch_tree_is_erased_by_the_start_that_refused_it,a_partly_minted_warm_set_is_still_the_pools_to_drain}` | COVERED |

### 4.4 CLI command matrix

Each command must cover its documented output modes, stdout/stderr separation,
runtime exit `1`, parser misuse exit `2`, parsed local-semantic validation exit
`1`, daemon unavailability, malformed peer/input, and redaction where
applicable. Dry-run `probe` has no daemon boundary, so daemon and peer faults
are exercised with `--launch`; an unhealthy `doctor` emits its typed report
before exiting `1`; text `attach` consumes its one-use capability as a raw
terminal proxy while JSON/NDJSON intentionally return the sensitive metadata.
...
| CLI-02 | `start` handle/compatibility and launch mapping | `process_boundary.rs::start_maps_the_complete_launch_dto_and_output_shapes`; cross-command failure matrix; exact release Gate D rerun pending | OPEN-L3 |
| CLI-03 | `turn` acceptance/events/replay-gap/final result/cancel-on-abort | `process_boundary.rs::{turn_sigint_cancels_the_exact_active_turn_and_emits_no_success,turn_replay_gap_reuses_the_immutable_turn_id_and_commits_once,completed_turn_covers_text_and_json_result_boundaries,noncompleted_turn_results_never_emit_a_terminal_result_record}`; exact release Gate D rerun pending | OPEN-L3 |
| CLI-04 | `run` observational events then exactly one result only after cleanup proof | `process_boundary.rs::{run_sigint_cancels_then_force_closes_and_withholds_success,run_ndjson_commits_exactly_once_only_after_cleanup_proof,run_cleanup_failure_keeps_observations_but_withholds_result_marker,run_failed_and_cancelled_terminal_results_never_emit_success_commit_markers}`; exact release Gate D rerun pending | OPEN-L3 |
| CLI-05 | `inspect` exact generation snapshot | `process_boundary.rs::{inspect_cancel_close_and_attach_metadata_map_exact_dtos,operation_specific_text_json_and_ndjson_shapes_are_stable}`; cross-command failure matrix; exact release Gate D rerun pending | OPEN-L3 |
| CLI-06 | `cancel` exact turn and terminal outcomes | `process_boundary.rs::{inspect_cancel_close_and_attach_metadata_map_exact_dtos,cancel_recovery_failure_is_nonzero_and_withholds_success_in_every_mode}`; exact release Gate D rerun pending | OPEN-L3 |
| CLI-07 | `close` policy, retryable unconfirmed reap and tombstone replay | `process_boundary.rs::{inspect_cancel_close_and_attach_metadata_map_exact_dtos,close_and_probe_reject_success_responses_without_process_reap_proof}` plus public full-stack close/tombstone cells; exact release Gate D rerun pending | OPEN-L3 |
| CLI-08 | `attach` text proxy versus sensitive JSON/NDJSON capability | `process_boundary.rs::{text_attach_proxies_bytes_restores_terminal_and_never_leaks_capability,inspect_cancel_close_and_attach_metadata_map_exact_dtos}` plus full-stack CLI attach; exact release Gate D rerun pending | OPEN-L3 |
...
| CLI-10 | `probe` is dry-run by default, redacted, and `--launch/--keep` bounded | `process_boundary.rs::{probe_dry_run_is_redacted_and_launch_keep_are_bounded,probe_inspection_failure_force_closes_even_with_keep_and_combines_cleanup_failure}`; exact release Gate D rerun pending | OPEN-L3 |
...
| CLI-12 | `--profile`/`--profile-file` (and their `PMUX_PROFILE`/`PMUX_PROFILE_FILE` fallbacks) expand a profile into the start DTO entirely client-side; an explicit flag replaces a profile scalar and says so on stderr while unstated scalars still inherit and lists append profile-first; `--profile` without a file is a typed local failure naming "performs no profile discovery"; a file without `--profile` selects nothing and does not read the file; the RETIRED spellings `--agent-file`, `PMUX_AGENT` and `PMUX_AGENT_FILE` are refused with the new one NAMED and are never silently aliased, because `--agent` now means the stored server agent; the permission bypass is reachable from the command line as `--permission-mode dangerously-skip-permissions` | L0 `bin/pmux/src/cli.rs::tests::{an_agent_profile_expands_into_the_start_dto_before_the_request_is_framed,explicit_flags_override_profile_scalars_loudly_and_append_to_profile_lists,agent_selection_is_explicit_on_both_sides,the_retired_profile_spellings_are_refused_by_name,extra_args_and_the_dangerous_bypass_are_reachable_from_the_command_line}`; expansion site `cli.rs::resolve_agent_profile`, refusal site `cli.rs::refuse_retired_profile_spellings`, override notes `cli.rs::override_scalar` | COVERED |
| CLI-13 | `--env-passthrough KEY` is the reachable escape hatch for the Section 4.5 allowlist: the flag takes a variable **name**, reads that name's value from the CLI's own process environment, and places the pair in `EnvironmentSpec.set`, so the value never appears in the `pmux` argv; the flag repeats; a name absent from the caller environment and an invalid environment name are typed local validation failures before daemon contact (exit `1`, not exit `2`); the resulting `set` entry bypasses the allowlist but is still subject to the `subscription` policy pass, so an Anthropic credential name cannot be reintroduced this way | `bin/pmux/src/cli.rs` flag definition and `set` construction, with the client-side expansion tests in `cli.rs::tests`; `bin/pmux/tests/process_boundary.rs` pass-through cases; `bin/pmux/tests/cli_contract_matrix.rs::{parser_misuse_is_exit_two_for_every_command,parsed_local_validation_is_exit_one_for_every_command}`; the policy interaction itself is owned by `S-26`; exact release Gate D rerun pending | OPEN-L3 |
| CLI-14 | `probe` is the audit surface for the allowlist: its report names every variable the caller offered that the launched child will not receive, whichever step dropped it, as **names only**. No environment value is serialized into a probe report in any output mode, and the existing redacted `snapshot`/`set` variable counts are unchanged. Dry-run `probe` has no daemon boundary, so the listing must hold both without `--launch` and with it | `bin/pmux/src/main.rs::{ProbeReport,EnvironmentRemovals,sanitized_start_request}` fed by `bin/pmux/src/cli.rs::dropped_environment_names`; `bin/pmux/tests/process_boundary.rs::probe_dry_run_is_redacted_and_launch_keep_are_bounded`; `bin/pmux/src/main.rs::tests::probe_never_serializes_environment_values`; removal-set completeness in the daemon is owned by `S-25`, and the client-side reimplementation of the allowlist that `dropped_environment_names` requires is itself a drift channel that needs a fence against `crates/service/src/claude_launch.rs::inherited_from_snapshot`; exact release Gate D rerun pending | OPEN-L3 |
| AGT-01 | An agent stores LAUNCH POLICY and never a resource: `AgentSpec` has no `cwd`, no `config_isolation`, no session identity, no prompt and no `environment.snapshot`, and the absence is STRUCTURAL rather than documented -- `AgentEnvironmentSpec` deletes the field. `environment.set` may not name any `CONFIG_ROOT_ENV_DOORS` entry, for EVERY cell, because an agent id is a name every session started from it shares | L0 `crates/service/tests/agent_resource.rs::{require_config_isolation_refuses_a_start_and_never_supplies_a_root,an_agent_may_not_set_a_variable_that_moves_the_childs_configuration_root}`; door table `claude_launch::CONFIG_ROOT_ENV_DOORS`, walked rather than copied | COVERED |
| AGT-02 | Resolution is a PURE function of `(AgentSpec, per-session fields)` and its output is a `StartSessionRequest` nothing downstream can distinguish from an inline one; `agent` is cleared and `claude` is present. This is what keeps `spec.md` Sec. 4.4 literally true | L0 `crates/service/tests/agent_resource.rs::agent_resolution_is_a_pure_function_of_the_spec_and_the_session_fields`, asserted as a wire-form equality against a hand-written inline request | COVERED |
| AGT-03 | A stored version is IMMUTABLE and a running session pins by value: an `update_agent` mints `version+1`, never rewrites `<version>.json`, and `inspect_session` on a session started under v1 still reports v1's `config_digest` after the store moves to v2 | L0 `crates/service/tests/agent_resource.rs::an_update_mints_a_new_version_and_never_rewrites_an_old_one`; L3 MEASURED live -- the v1 session's child argv still carried `--permission-mode plan` after the store moved to a v2 with `dontAsk` | COVERED |
| AGT-04 | `expected_version` is a REQUIRED fence: any value that is not the head is `id_conflict`, including one stale by exactly one revision and including one whose submitted spec is byte-identical to the head, and nothing is ever answered as "already landed" | L0 `crates/service/tests/agent_resource.rs::a_stale_update_fence_is_a_conflict_and_is_never_answered_as_already_applied` | COVERED |
| AGT-05 | The store is held to the socket directory's bar: 0700 directories and 0600 files FROM BIRTH at every level pmux creates, an operator's non-private tree REFUSED at boot naming what is wrong AND what would be right, the mode re-checked on every READ -- of the BYTES, via `O_NOFOLLOW` plus `fstat` on the open handle, not of the name in front of them, and of the per-agent DIRECTORY as well as its files -- and never a re-permission. The guard used to `symlink_metadata` the name and then `std::fs::read` through it, and a symlink's own mode is `umask`-dependent: MEASURED under `umask 077`, a `1.json` that was a symlink to a `0666` file outside the store was accepted and launched | L0 `crates/service/tests/agent_resource.rs::{the_store_is_private_at_every_level_and_in_every_file_it_creates,a_widened_store_is_refused_and_is_left_exactly_as_the_operator_left_it,a_widened_agent_file_is_refused_when_it_is_read_and_not_only_at_boot,a_version_file_that_is_not_a_regular_file_this_store_wrote_is_refused,a_widened_agent_directory_is_refused_when_it_is_read_and_not_only_at_boot}` | COVERED |
| AGT-06 | Traversal is UNCONSTRUCTIBLE, not filtered: the path component is always a daemon-minted canonical UUID and `spec.name` never reaches the filesystem, walked over the hostile labels `agent_profile::validate_agent_name` admits (`..`, `.`, `...`, `a..b`, `-`, `../../etc`) | L0 `crates/service/tests/agent_resource.rs::a_stored_agent_is_named_by_a_minted_uuid_and_never_by_its_label` | COVERED |
| AGT-07 | Containment NARROWS and never widens: `workspace_root` bounds a cwd and never supplies one, is asked in ONE direction so a cwd that CONTAINS the root is refused, and is decided on the resource so a symlink into the bound is admitted while a name prefix is not | L0 `crates/service/tests/agent_resource.rs::containment_bounds_a_cwd_and_never_supplies_or_widens_one`; predicate `claude_launch::directory_lies_within`; the COMPOSITION direction -- a cwd the existing rules already refuse stays refused under every `workspace_root`, including one that contains it -- in L0 `crates/service/src/native.rs::tests::containment_can_only_refuse_more_never_admit_more`, which the module doc had named since the resource shipped and which did not exist | COVERED |
| AGT-08 | Exactly one of `agent` and the inline launch fields, refused BY NAME on the wire, over a list DERIVED by intersecting the serialized leaf paths of the two types; `environment.snapshot` survives with no exception list; the refusal is symmetric on `Serialize` over ALL NINE derived paths, so a Rust embedder cannot have a value silently dropped. The serializer used to hand-write FIVE arms and justify the omission by saying `claude`, `environment.set`/`unset` and `cell` were "refused by name" -- in `Deserialize`, which no in-process caller runs, since `validate_v1_serializable` only serializes. MEASURED against that shape: an embedder sending `cell: "minified"` beside a `full` agent was accepted and launched `full`. Both presence tables now return `[(&str, bool); AGENT_SUPPLIED_START_PATHS.len()]`, so a path added to the list stops both compiling | L0 `crates/protocol/tests/v1_wire.rs::{the_agent_supplied_start_paths_are_exactly_the_serialized_leaf_collision,a_start_naming_an_agent_is_refused_by_name_for_every_supplied_path,a_start_carrying_launch_policy_without_an_agent_still_serializes,a_start_must_name_either_an_inline_launch_or_an_agent}`; L0 `bin/pmuxd/src/handler.rs::tests::a_decode_refusal_pmux_wrote_reaches_the_caller_and_one_serde_wrote_does_not`; L3 MEASURED live over a real socket | COVERED |
| AGT-09 | Redaction holds and the digest still identifies: no `get_agent`/`list_agents` frame carries an environment value or an inline document body, the system prompt is deliberately NOT redacted, and the reported `config_digest` is one a caller can recompute over the spec it SUBMITTED -- computed in the test with `sha256` directly, because asking the production function for the expected value cannot catch a change to it | L0 `crates/service/tests/agent_resource.rs::redaction_hides_values_while_the_digest_still_identifies_the_configuration`; L3 MEASURED live -- `PMUX_AGENT_MARKER` came back as `sha256:227e6a...` while `1.json` on disk held `reviewer-v1` | COVERED |
| AGT-10 | A stored agent is one that can START: `create_agent` runs the service's OWN `validate_v1_terminal_support` and `validate_public_start_retention`, so a `one_shot` retention or a reserved terminal identity is refused where the caller can fix it; `cell: minified` with `require_config_isolation: false` is refused rather than silently overridden | L0 `crates/service/tests/agent_resource.rs::{a_spec_a_start_would_refuse_is_refused_at_create,a_minified_agent_that_does_not_require_its_own_root_is_refused_at_create}` | COVERED |
| AGT-11 | `--agent` and any inline launch flag the caller TYPED is refused in the CALLER'S OWN VOCABULARY, over a classification of `LaunchArgs` that clap's own argument ids are partitioned against, so a flag added to `start` and dropped from both classes is red. A launch value clap filled from the ENVIRONMENT is a different event: `env` is clap's lowest precedence source below argv, so `--agent` overrides it and REPORTS the override by the variable's own name, read from clap's argument metadata. MEASURED before the split, `PMUX_MODEL=opus pmux start --agent ...` was refused "cannot be combined with --model" -- naming a flag the caller never typed, and locking anyone with that variable exported in a shell rc out of `--agent` for good | L0 `bin/pmux/src/cli.rs::tests::{every_launch_flag_is_either_per_session_or_named_as_agent_supplied,naming_a_stored_agent_and_an_inline_launch_flag_is_refused_by_flag_name,a_launch_value_from_the_environment_is_overridden_by_agent_and_reported_by_variable}`; L2 real process `bin/pmux/tests/native_cli.rs::an_exported_launch_variable_is_overridden_by_agent_and_a_typed_flag_is_still_refused` | COVERED |
| AGT-12 | Path B never gains an agent reference: `run_stateless` with `{"agent_id": ...}` is refused, and the pool class key stays `(model, effort)` | L0 `crates/protocol/tests/v1_wire.rs::path_b_refuses_an_agent_reference_by_name` | COVERED |
| AGT-13 | Version publication is ATOMIC and EXCLUSIVE: the bytes are written and `fsync`ed under a temporary name no other writer shares, and `link(2)` gives the finished inode its real name and fails with `EEXIST` rather than replacing -- so naming the file and refusing to overwrite one are the SAME syscall and two concurrent `update_agent` calls on one fence publish one version or neither. The `expected_version` comparison is a courtesy for the ordinary stale caller and cannot be the fence: `handler.rs` serves 64 connections at once and there is no lock between reading `head` and writing. MEASURED before this, 25 rounds of two concurrent updates on one fence: 7 left `head` naming a file that no longer parsed (`trailing characters at line 1 column 1497`, which also took `list_agents` down for the whole store) and 13 more answered the winner a `config_digest` the store did not hold | L0 `crates/service/tests/agent_resource.rs::concurrent_updates_on_one_fence_publish_one_version_and_never_a_torn_one`, 25 rounds, asserting one winner, an `id_conflict` loser naming the version that now exists, a readable head, a readable pinned v1, an empty `unreadable`, and the winner's digest equal to the store's | COVERED |
| AGT-14 | ONE BAD RECORD LOSES ITSELF AND NOTHING ELSE: `list_agents` reports every record it could not read in `AgentList::unreadable`, by id, with the refusal `get_agent` would have given for it, and never drops one and never answers the whole listing with it. Walked over four states the store can genuinely be in -- a widened `head`, a torn version file, a `head` naming a version that was never minted, and a record with no `head` at all. The field is omitted from the wire when empty, so an ordinary listing's bytes are what every release before it sent; both shipped clients validate it, and `pmux agent list` prints it. This is also what makes `no agent <id>`'s own recommendation, "list the stored agents with `pmux agent list`", reachable in the state it is offered | L0 `crates/service/tests/agent_resource.rs::one_unreadable_record_is_reported_by_id_and_never_takes_the_listing_down`; L0 `crates/protocol/tests/v1_wire.rs::a_listing_reports_unreadable_records_and_omits_the_field_when_there_are_none`; TS `clients/typescript/tests/client.test.mjs`, Python `clients/python/tests/test_client.py` | COVERED |
| AGT-15 | `create_agent` publishes an agent WHOLE or not at all: it is assembled under a staging name that is not a UUID and becomes visible at its real name in one `rename(2)`. The first shape created the UUID directory and then wrote into it, so every interrupted create left a record with no `head` -- which is exactly the record `list_agents` then had to read. Asserted from OUTSIDE, by a reader walking the store while 200 creates run: the green direction is deterministic, because the UUID name comes into existence with the whole agent already behind it | L0 `crates/service/tests/agent_resource.rs::a_created_agent_is_never_observable_half_made` | COVERED |
| AGT-16 | The MCP surface DELIVERS the refusal its own tool description promises: a start naming both an agent and a launch field is answered `invalid_config` with the colliding path NAMED, forwarded through the same `caller_actionable_decode_refusal` span `pmuxd` forwards, and a refusal serde wrote out of the caller's own values stays the content-free `invalid_arguments`. The input schema STATES the rule as `dependentSchemas` on `agent` plus an `anyOf` over `claude`/`agent`, derived from `agent_supplied_start_paths()`, so a model reads it rather than discovering it by failing. MEASURED before this, four cases all answered `arguments do not match the native pmux protocol schema` with `"kind": "invalid_arguments"` and no field name, and the schema listed the eight fields as plain optional siblings | L0 `bin/pmux-mcp/src/tools.rs::tests::{a_both_modes_refusal_reaches_an_mcp_caller_naming_the_colliding_field,a_decode_failure_serde_wrote_is_never_forwarded_to_an_mcp_caller,the_start_schema_forbids_exactly_the_agent_supplied_paths}` | COVERED |
| AGT-17 | `run_once` decides its session's retention and RESOLUTION DOES NOT DISCARD IT: the decision travels beside the request as `Retention::ForcedOneShot` and is applied AFTER the agent is resolved, in the one function that owns both steps. It used to be written into the request as a value before the start ran, and resolution replaced the whole launch policy with the stored agent's -- so `pmux run --agent` registered the agent's idle TTL instead of `None`, a value pmux wrote and pmux discarded inside its own path | L0 `crates/service/src/native.rs::tests::{a_forced_one_shot_survives_the_agent_it_resolves,run_once_is_the_only_start_that_forces_one_shot_retention}`, the first driving `resolve_agent_and_retention` against a real store on disk with no daemon, PTY or Claude, the second a source census of the call sites those need a Claude process to reach | COVERED |
...
| BIN-02 | `pmux-mcp` | Initialize/list and all eight tools over stdio; structuredContent once; strict errors/frames/output | all five tests in `bin/pmux-mcp/tests/stdio_blackbox.rs`, including `real_stdio_bounds_oversized_output_and_recovers_on_one_stream`; exact release Gate D rerun pending | OPEN-L3 |
| BIN-03 | `claude-p` | Positional/stdin; marker consumption; text/json/stream-json provenance; rejection/exit/cleanup | `bin/claude-p/tests/facade_blackbox.rs`; exact release Gate D rerun pending | OPEN-L3 |
...
| BIN-07 | Full stack | Exact release binaries + private rmux + deterministic interactive fake Claude; all ten v1 methods, including the Path B minified cell driven through `clear_session` | `crates/e2e/tests/full_stack.rs`; integrated exact release Gate D rerun pending | OPEN-L3 |
...
| CL-02 | Every Rust typed method sends exact request and requires matching result | `crates/client/tests/v1_golden.rs::all_ten_typed_methods_send_exact_golden_requests_and_accept_matching_results`; `crates/client/tests/fake_uds.rs::every_typed_method_rejects_contextually_mismatched_results` | COVERED |
...
| CL-06 | MCP maps exactly eight strict tools to the same DTOs; attach metadata is sensitive and not consumed | `bin/pmux-mcp/src/tools.rs` unit tests and `bin/pmux-mcp/tests/stdio_blackbox.rs::real_stdio_maps_all_eight_tools_to_exact_native_requests`; exact release Gate D rerun pending | OPEN-L3 |
| CL-07 | `claude-p` remains one thin `run_once` facade; print-shaped flags never reach Claude | `bin/claude-p/tests/facade_blackbox.rs` and full-stack child argv attestation; exact release Gate D rerun pending | OPEN-L3 |
| CL-08 | The agent-profile document loader composes fail-closed: scalars replace and absent inherits, lists append parent-first, `extends` is one chain bounded at depth 4 with named cycle rejection, JSON `null` is a parse error outside inline documents, unknown keys are rejected at every level, per-invocation keys (`cwd`, identity, prompt, deadline) are rejected by name, duplicate keys never resolve to the last definition, and values reserved-but-unimplemented in v1 fail at expansion instead of at launch | `crates/client/tests/agent_profile.rs` — all sixteen tests, including `{extends_chain_replaces_scalars_and_appends_lists_parent_first,cyclic_extends_chains_are_named_and_rejected,extends_depth_is_bounded_at_four_and_never_silently_truncated,a_literal_null_is_a_parse_error_rather_than_an_unset_operator,inline_documents_keep_their_own_nulls,unknown_keys_are_rejected_at_every_level,per_invocation_fields_are_rejected_by_name,reserved_but_unimplemented_values_fail_at_expansion_not_at_launch,supported_tagged_policies_deserialize_into_the_protocol_types,duplicate_agent_names_are_rejected_instead_of_last_one_winning,profile_paths_are_explicit_and_documents_are_versioned}` | COVERED |
| CL-09 | The loader applies the confidentiality checks the service cannot: any inline settings/MCP document forces the profile file itself to be owner-only, every referenced `{"source":"file"}` entry must be absolute and satisfy `mode & 0o077 == 0`, plugin directories must be absolute, and `require_env` asserts presence over the effective `snapshot - unset + set` environment without ever reading a value, warning when `auth_policy: subscription` or a `transparent` terminal profile would strip a named variable from the child | `crates/client/tests/agent_profile.rs::{a_document_carrying_an_inline_secret_must_be_owner_only,referenced_config_files_must_be_owner_only_and_absolute,require_env_asserts_presence_without_ever_reading_the_value,require_env_warns_when_the_resolved_policies_would_strip_the_name,require_env_follows_the_effective_environment_patch_order}`; enforcement at `crates/client/src/agent_profile.rs::expand` (the inline-document check), `::validate_resolved_paths`, `::verify_required_environment` | COVERED |
...
| Streaming prompt input | Native incremental streaming is `OUT-OF-SCOPE` (`PLAT-10`). `claude-p` flags claiming it are a typed/parser rejection contract owned by `S-22`, currently `OPEN-L3` pending Gate D. |
...
| Automatic trust/login/permission/update/quota answers | A positive no-write/typed-needs-input invariant, not an unsupported request. `S-20` must prove zero automatic answer bytes through real PTY composition. |
| Transparent restart recovery | `OUT-OF-SCOPE` (`PLAT-09`). `S-21` separately proves no actor reconstruction or prompt reinjection and proves explicit known-UUID resume. |
...
| Built-in broad Claude-version admission | A positive fail-closed compatibility invariant owned by `S-04`: the source registry is empty and only exact operator-supplied cells admit launch. |
...
Gate A closes only when every deterministic row is `COVERED`, tested
`REJECTED`, or explicitly reviewed `OUT-OF-SCOPE`, all commands above pass,
residue is empty, and independent review finds no missing/false-positive row.
The canonical source and exact release binaries are then hashed together. Any
canonical source change invalidates the freeze and all later evidence.
...
After macOS evidence review, the exact Gate C entry point is:
...
resolve one — so the operator supplies it; `tools/linux-docker/run.sh:49` is the
exact accepted form. Docker authorization is limited to the unique builders,
