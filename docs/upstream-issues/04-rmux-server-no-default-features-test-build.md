...
Context: I depend on `rmux-server` with `default-features = false` because I drive panes over the
local API and have no use for the web share surface, and the check I run is exactly
...
feature is on, but neither the test nor its module carries a matching gate.
...
On macOS 15.7.7 arm64, `rustc 1.97.1 (8bab26f4f 2026-07-14)`:
...
One line, on the single affected test:
...
With that diff applied on an otherwise-pristine 0.10.0 tree I ran:
...
Happy to open a PR with exactly that one-line change if you would like it as one.
